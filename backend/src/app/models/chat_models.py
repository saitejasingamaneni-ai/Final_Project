...
class Chat(Base):
    ...
    creator = relationship("src.app.models.user_models.User", backref="created_chats", foreign_keys=[creator_id])
    participants = relationship("src.app.models.chat_models.ChatParticipant", back_populates="chat", cascade="all, delete-orphan")
...
class ChatParticipant(Base):
    ...
    # After:
    user = relationship("src.app.models.user_models.User", backref="chat_memberships")
    # Before: chat = relationship("Chat", back_populates="participants")
    # After (already string, but confirming full path):
    chat = relationship("src.app.models.chat_models.Chat", back_populates="participants")
